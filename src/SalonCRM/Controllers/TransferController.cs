...
namespace SalonCRM.Controllers
{
    ...
    public class TransferController : Controller
    {
        ...
        public ActionResult Index(CardRepQModel qmodel)
        {
            ...
            qmodel.TransferList = GetTranferList(qmodel);
            return View(qmodel);
        }
        /// <summary>
        /// 转卡
        /// </summary>
        /// <returns></returns>
        public ActionResult TransferList(CardRepQModel qmodel)
        {
            ViewData["Name"] = qmodel.Name;
            ViewData["BranchId"] = qmodel.BranchId;
            ViewData["CardNo"] = qmodel.CardNo;
            ViewData["Mobile"] = qmodel.Mobile;
            return PartialView("TransferList", GetTranferList(qmodel));
        }

        public ActionResult TransferDetails(int id)
        {
            int hostId = GlobalContext.Current.UserHost.HostID;
            CardTransferViewModel model = (from vm in dbcontent.EventLogs.Where(t => t.LogId == id)
                                           join br in dbcontent.Organs on vm.BranchId equals br.OrganID
                                           join mv in dbcontent.Members on vm.MemberId equals mv.MemberID
                                           select new CardTransferViewModel
                                           {
                                               LogId = vm.LogId,
                                               UserId = vm.UserId,
                                               UserName = dbcontent.Users.Where(t => t.Id == vm.UserId).FirstOrDefault().UserCnName,
        ...
        /// </summary>
        /// <returns></returns>
        private IList<CardTransferViewModel> GetTranferList(CardRepQModel qmodel)
        {
            int hostId = GlobalContext.Current.UserHost.HostID;
            var query = dbcontent.EventLogs.Where(t => t.HostId == hostId);
            ...
            IList<CardTransferViewModel> logs = (from vm in query
                                                 join br in dbcontent.Organs on vm.BranchId equals br.OrganID
        ...
        {
            var printable = GridViewExtension.CreatePrintableObject(GridViewSettings, GetTranferList(viewModel));
        ...
        static GridViewSettings GetGridViewSettings()
        {
            ...
            settings.Columns.Add("MemberId", "客户");
            settings.Columns.Add("CardNo", "卡号");
            settings.Columns.Add("BranchName", "门店");
            settings.Columns.Add("UserName", "操作人");
            settings.Columns.Add("OriginalProjects", "原项目");
            settings.Columns.Add("OriginalPrjAmt", "原项目金额");
            settings.Columns.Add("NewProjects", "新项目");
            settings.Columns.Add("NewPrjAmt", "新项目金额");
