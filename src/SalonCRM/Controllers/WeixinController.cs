...
namespace Senparc.Weixin.MP.Sample.Controllers
{
    ...
    public partial class WeixinController : Controller
    {
        ...
        public ActionResult Get(PostModel postModel, string echostr)
        {
            ...
            if (CheckSignature.Check(postModel.Signature, postModel.Timestamp, postModel.Nonce, bag.Token))
            {
                return Content(echostr); //返回随机字符串则表示验证通过
            }
            else
            {
                return Content("failed:" + postModel.Signature + "," + MP.CheckSignature.GetSignature(postModel.Timestamp, postModel.Nonce, bag.Token) + "。" +
        ...
        public ActionResult Post(PostModel postModel)
        {
            ...
            postModel.Token = bag.Token;//根据自己后台的设置保持一致
            postModel.EncodingAESKey = bag.EncodingAESKey;//根据自己后台的设置保持一致
            postModel.AppId = bag.AppId;//根据自己后台的设置保持一致
            ...
            var messageHandler = new CustomMessageHandler(Request.InputStream, postModel, bag.Secret, maxRecordCount, bag.HostID);
        ...
        public ActionResult MiniPost(PostModel postModel)
        {
            ...
            postModel.Token = bag.Token;
            postModel.EncodingAESKey = bag.EncodingAESKey;//根据自己后台的设置保持一致
            postModel.AppId = bag.AppId;//根据自己后台的设置保持一致

            var messageHandler = new CustomMessageHandler(Request.InputStream, postModel, bag.Secret, 10, bag.HostID);
